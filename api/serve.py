...
def _mock_confidence(question: str, schema: str, sql: str) -> float:
    ...
    strong_patterns = [
        ...
        r'\bwhat\b', r'\bwhich\b', r'\bwhere\b', r'\bwho\b',
    ]
    ...
    if tables:
        ...
        if any(re.search(rf'\b{re.escape(t)}\b', q) for t in table_names):
            score += 0.15
        elif any(
            re.search(rf'\b{re.escape(v)}\b', q)
            for t in table_names
            for v in (t.rstrip("s") if t.endswith("s") and not t.endswith("ss") else t, t + "s")
        ):
            score += 0.10
        else:
            score -= 0.15

    if tables and any(re.search(rf'\b{re.escape(col.lower())}\b', q) for _, cols in tables for col in cols):
        score += 0.15
    if len(words) >= 5:
        score += 0.05
...
def _generate_smart_sql(question: str, schema: str) -> tuple[str, float, float]:
    ...
    def _extract_where_clause(q: str, text_cols: list, columns: list) -> str | None:
        ...
        for col in text_cols:
            pattern = rf'\b(?:named|called)\s+["\']?([A-Za-z]\w+)["\']?'
            m = re.search(pattern, q)
            if m:
                val = m.group(1).strip()
                if val.lower() not in skip_words:
        ...
        for col in text_cols:
            synonyms = col_synonyms.get(col.lower(), [])
            for syn in synonyms:
                pattern = rf'\b(?:in|from|of|for|with)\s+(?:the\s+)?["\']?([A-Za-z]\w+)["\']?\s*$'
                m = re.search(pattern, q)
                if m:
                    val = m.group(1).strip()
    ...
    if join_sql:
        ...
        where = _extract_where_clause(q, text_cols, columns)
        if where:
            sql += f" WHERE {where}"
